...
import enum
from Alessio.group0_utils_alessio import *
from operator import sub

TEAM_NAME = 'Weighted graph'
...
def move_attack(bot, state, was_recur = False):
    ...
    if (state.target[bot.turn] is None) or (state.target[bot.turn] not in bot.enemy[0].food):
        ...
        food_dist = calculate_all_paths(bot.position, bot.enemy[0].food, graph_with_enemies)
    ...
    if all(enemy_nearby):
        ...
        food_dist = calculate_all_paths(bot.position, bot.enemy[0].food, graph_with_enemies)
        enemy_0_food_dist = calculate_all_paths(bot.enemy[0].position, bot.enemy[0].food, state.nx_G)
        enemy_1_food_dist = calculate_all_paths(bot.enemy[1].position, bot.enemy[0].food, state.nx_G)
        ...
        if np.max(food_enemy_diff) < 0 and not bot.position in bot.homezone:
            boundary_dist = calculate_all_paths(bot.position, state.home_boundaries, graph_with_enemies)
            enemy_0_boundary_dist = calculate_all_paths(bot.enemy[0].position, state.home_boundaries, state.nx_G)
            enemy_1_boundary_dist = calculate_all_paths(bot.enemy[1].position, state.home_boundaries, state.nx_G)
            ...
            state.target[bot.turn] = bot.enemy[0].food[min_dist_idx]
            next_pos = next_step(bot.position, state.target[bot.turn], state.nx_G)
    elif any(enemy_nearby):
        enemy = bot.enemy[np.argmax(enemy_nearby)]
        bot.say("AHAHAHAHAH!".format(np.argmax(enemy_nearby)))
        food_dist = calculate_all_paths(bot.position, bot.enemy[0].food, graph_with_enemies)
        enemy_food_dist = calculate_all_paths(enemy.position, bot.enemy[0].food, state.nx_G)
        food_enemy_diff = list(map(sub, enemy_food_dist, food_dist))
        if np.max(food_enemy_diff) < 0 and not bot.position in bot.homezone:
            bot.say("Go away BOT {0}!".format(np.argmax(enemy_nearby)))
            boundary_dist = calculate_all_paths(bot.position, state.home_boundaries, graph_with_enemies)
            enemy_boundary_dist = calculate_all_paths(enemy.position, state.home_boundaries, state.nx_G)
            boundary_enemy_diff = list(map(sub, enemy_boundary_dist, boundary_dist))
            min_dist_idx = np.argmax(boundary_enemy_diff)
            next_pos = next_step(bot.position, state.home_boundaries[min_dist_idx], state.nx_G)
        else:
